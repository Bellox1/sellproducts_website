    ...
    <div class="container-fluid px-3 px-md-5 pt-0 pb-4">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-4 mb-md-5 gap-4 animate-in">
            <div class="glass-container p-4 rounded-5 border border-white border-opacity-50 w-100">
                <p class="text-secondary small ls-2 text-uppercase mb-0 fw-bold" style="letter-spacing: 4px;">Détails Transaction</p>
                <h1 class="display-5 display-md-3 fw-bold mb-0 text-dark">Bon de Commande #{{ $commande->id }}.</h1>
                <p class="fs-5 text-muted mt-3 fw-medium">Résumé complet des articles et informations de cette commande.</p>
            </div>
            <div class="pb-3 w-100 d-md-flex justify-content-end">
                <a href="{{ url()->previous() }}" class="btn btn-glass-auth shadow-sm fw-bold w-100 w-md-auto d-inline-block py-2">
        ...
        @if(session('success'))
            <div class="alert glass-container border-success border-opacity-25 text-success p-4 rounded-5 mb-4 animate-in">
                                    ...
                                    <tr class="border-top">
                                        <td class="py-4">
                                            ...
                                            @if(isset($produit['description']))
                                                <div class="text-secondary small">{{ Str::limit($produit['description'], 50) }}</div>
                                            @endif
                                        </td>
                                        <td class="py-4 text-center text-secondary">{{ number_format($produit['prix'] ?? 0, 2) }} €</td>
                                        <td class="py-4 text-center">
                                            <span class="badge glass-pill text-dark px-3 py-1 rounded-pill">{{ $produit['quantite'] ?? 0 }}</span>
                                        </td>
                                        <td class="py-4 text-end fw-bold text-dark">{{ number_format($produit['sous_total'] ?? 0, 2) }} €</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="col-lg-4">
                <div class="glass-container p-4 p-md-5 rounded-5 shadow-sm border border-white mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <h2 class="fw-bold text-primary">{{ number_format($commande->total, 2) }} €</h2>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex flex-column gap-2">
                            ...
                            <div class="dropdown">
                                <button class="btn btn-sm btn-glass-auth rounded-pill px-4 dropdown-toggle w-100" type="button" data-bs-toggle="dropdown">
                                    CHANGER LE STATUT
                                </button>
                    ...
                    @if($commande->client_email)
                        <div class="border-top border-white border-opacity-50 pt-4">
                            ...
                            <h6 class="text-dark fw-bold"><i class="bi bi-envelope me-2"></i>{{ $commande->client_email }}</h6>
                        </div>
                    @endif
                </div>
...
<style>
    ...
    .icon-box {
        ...
        border-radius: 12px; background: rgba(244, 118, 104, 0.1); color: #f47668;
    }
